...
const AdminDashboard = () => {
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-3 shadow-glow">
          <p className="text-sm text-zinc-400 mb-1">{payload[0].payload.name}</p>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-zinc-900 border border-zinc-800 rounded-2xl p-7 shadow-sm">
          <h2 className="text-base font-semibold text-white mb-8 flex items-center gap-2">
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <XAxis dataKey="name" stroke="#8F9197" axisLine={false} tickLine={false} tick={{ fontSize: 12, fontWeight: 500 }} dy={10} />
                <YAxis stroke="#8F9197" axisLine={false} tickLine={false} tick={{ fontSize: 12 }} />
                <Tooltip content={<CustomTooltip />} cursor={{ fill: '#272A30', opacity: 0.4 }} />
                <Bar dataKey="value" fill="#5E6AD2" radius={[6, 6, 6, 6]} barSize={48}>
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
            ...
            {stats.recentActivity && stats.recentActivity.length > 0 ? (
              stats.recentActivity.map((activity) => (
                <div key={activity.id} className="flex gap-4 group">
                  <div className="w-9 h-9 rounded-full bg-border flex items-center justify-center flex-shrink-0 text-xs font-bold text-white group-hover:bg-indigo-500 group-hover:text-white transition-colors">
